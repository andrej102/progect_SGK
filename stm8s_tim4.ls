...
1291  01ea 4f            	clr	a
1293  01eb               L574:
1294                     ; 333   return ((FlagStatus)bitstatus);
1298  01eb 85            	popw	x
1299  01ec 81            	ret	
1335                     ; 343 void TIM4_ClearFlag(TIM4_FLAG_TypeDef TIM4_FLAG)
1335                     ; 344 {
1336                     	switch	.text
1337  01ed               _TIM4_ClearFlag:
1339  01ed 88            	push	a
1340       00000000      OFST:	set	0
1343                     ; 346     assert_param(IS_TIM4_GET_FLAG_OK(TIM4_FLAG));
1345  01ee 4a            	dec	a
1346  01ef 270e          	jreq	L002
1347  01f1 ae015a        	ldw	x,#346
1348  01f4 89            	pushw	x
1349  01f5 5f            	clrw	x
1350  01f6 89            	pushw	x
1351  01f7 ae0000        	ldw	x,#L56
1352  01fa cd0000        	call	_assert_failed
1354  01fd 5b04          	addw	sp,#4
1355  01ff               L002:
1356                     ; 349     TIM4->SR1 = (uint8_t)(~TIM4_FLAG);
1358  01ff 7b01          	ld	a,(OFST+1,sp)
1359  0201 43            	cpl	a
1360  0202 c75342        	ld	21314,a
1361                     ; 351 }
1364  0205 84            	pop	a
1365  0206 81            	ret	
1426                     ; 360 ITStatus TIM4_GetITStatus(TIM4_IT_TypeDef TIM4_IT)
1426                     ; 361 {
1427                     	switch	.text
1428  0207               _TIM4_GetITStatus:
1430  0207 88            	push	a
1431  0208 89            	pushw	x
1432       00000002      OFST:	set	2
1435                     ; 362     ITStatus bitstatus = RESET;
1437                     ; 364   uint8_t itstatus = 0x0, itenable = 0x0;
1441                     ; 367   assert_param(IS_TIM4_IT_OK(TIM4_IT));
1443  0209 4a            	dec	a
1444  020a 270e          	jreq	L012
1445  020c ae016f        	ldw	x,#367
1446  020f 89            	pushw	x
1447  0210 5f            	clrw	x
1448  0211 89            	pushw	x
1449  0212 ae0000        	ldw	x,#L56
1450  0215 cd0000        	call	_assert_failed
1452  0218 5b04          	addw	sp,#4
1453  021a               L012:
1454                     ; 369   itstatus = (uint8_t)(TIM4->SR1 & (uint8_t)TIM4_IT);
1456  021a c65342        	ld	a,21314
1457  021d 1403          	and	a,(OFST+1,sp)
1458  021f 6b01          	ld	(OFST-1,sp),a
1460                     ; 371   itenable = (uint8_t)(TIM4->IER & (uint8_t)TIM4_IT);
1462  0221 c65341        	ld	a,21313
1463  0224 1403          	and	a,(OFST+1,sp)
1464  0226 6b02          	ld	(OFST+0,sp),a
1466                     ; 373   if ((itstatus != (uint8_t)RESET ) && (itenable != (uint8_t)RESET ))
1468  0228 7b01          	ld	a,(OFST-1,sp)
1469  022a 2708          	jreq	L345
1471  022c 7b02          	ld	a,(OFST+0,sp)
1472  022e 2704          	jreq	L345
1473                     ; 375     bitstatus = (ITStatus)SET;
1475  0230 a601          	ld	a,#1
1478  0232 2001          	jra	L545
1479  0234               L345:
1480                     ; 379     bitstatus = (ITStatus)RESET;
1482  0234 4f            	clr	a
1484  0235               L545:
1485                     ; 381   return ((ITStatus)bitstatus);
1489  0235 5b03          	addw	sp,#3
1490  0237 81            	ret	
1527                     ; 391 void TIM4_ClearITPendingBit(TIM4_IT_TypeDef TIM4_IT)
1527                     ; 392 {
1528                     	switch	.text
1529  0238               _TIM4_ClearITPendingBit:
1531  0238 88            	push	a
1532       00000000      OFST:	set	0
1535                     ; 394     assert_param(IS_TIM4_IT_OK(TIM4_IT));
1537  0239 4a            	dec	a
1538  023a 270e          	jreq	L022
1539  023c ae018a        	ldw	x,#394
1540  023f 89            	pushw	x
1541  0240 5f            	clrw	x
1542  0241 89            	pushw	x
1543  0242 ae0000        	ldw	x,#L56
1544  0245 cd0000        	call	_assert_failed
1546  0248 5b04          	addw	sp,#4
1547  024a               L022:
1548                     ; 397     TIM4->SR1 = (uint8_t)(~TIM4_IT);
1550  024a 7b01          	ld	a,(OFST+1,sp)
1551  024c 43            	cpl	a
1552  024d c75342        	ld	21314,a
1553                     ; 398 }
1556  0250 84            	pop	a
1557  0251 81            	ret	
1570                     	xdef	_TIM4_ClearITPendingBit
1571                     	xdef	_TIM4_GetITStatus
1572                     	xdef	_TIM4_ClearFlag
1573                     	xdef	_TIM4_GetFlagStatus
1574                     	xdef	_TIM4_GetPrescaler
1575                     	xdef	_TIM4_GetCounter
1576                     	xdef	_TIM4_SetAutoreload
1577                     	xdef	_TIM4_SetCounter
1578                     	xdef	_TIM4_GenerateEvent
1579                     	xdef	_TIM4_ARRPreloadConfig
1580                     	xdef	_TIM4_PrescalerConfig
1581                     	xdef	_TIM4_SelectOnePulseMode
1582                     	xdef	_TIM4_UpdateRequestConfig
1583                     	xdef	_TIM4_UpdateDisableConfig
1584                     	xdef	_TIM4_ITConfig
1585                     	xdef	_TIM4_Cmd
1586                     	xdef	_TIM4_TimeBaseInit
1587                     	xdef	_TIM4_DeInit
1588                     	xref	_assert_failed
1589                     .const:	section	.text
1590  0000               L56:
1591  0000 6c6962726172  	dc.b	"libraries\src\stm8"
1592  0012 735f74696d34  	dc.b	"s_tim4.c",0
1612                     	end
